...
<!-- AIDEV-NOTE: Default spawn topology is a background workspace to avoid pane clutter in the caller tab. -->
Default to a new background workspace in the current working directory. Do not split the caller pane unless the user explicitly asks for pane-based topology.

Create the workspace, then read the first pane ID from the returned JSON (or by listing panes in that workspace if needed):

```bash
herdr workspace create --no-focus --label "reviewer"
herdr pane list --workspace <returned-workspace-id>
```

Read `pane_id` from that workspace and use it as the control target. Give it a useful label, then start the requested agent by running only its normal executable so its interactive TUI opens:
...
## Run an ordinary command in another workspace

Create a background workspace and read its `pane_id`:

```bash
herdr workspace create --no-focus --label "cmd"
herdr pane list --workspace <returned-workspace-id>
```

Run and inspect the command in that workspace pane:
...
If the user explicitly asks for pane splits, tabs, or worktrees, discover that command group and use returned IDs. Otherwise keep the default new-workspace topology for spawned agents and commands.
