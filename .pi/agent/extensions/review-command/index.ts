...
export default function reviewCommandExtension(pi: ExtensionAPI): void {
	...
	type ReviewMode = "default" | "base-diff";

	async function startReviewFlow(args: string, ctx: ExtensionContext, mode: ReviewMode): Promise<void> {
		const commandName = mode === "base-diff" ? "/review-base-diff" : "/review";
		const prInput = normalizePrompt(args);
		if (!prInput) {
			ctx.ui.notify(
				`Usage: ${commandName} \"https://github.com/<owner>/<repo>/pull/<number>\" or ${commandName} <pr-number>`,
				"error",
			);
			return;
		}
		...
		if (!inputIsUrl && !inputIsNumber) {
			ctx.ui.notify(`${commandName} supports a GitHub PR URL or numeric PR number.`, "error");
			return;
		}

		const prUrl = inputIsUrl ? prInput : "";
		const prNumber = inputIsUrl ? extractGithubPrNumber(prInput) : prInput;
		const prSelector = inputIsUrl ? prUrl : prNumber;
		...
		// AIDEV-NOTE: Review commands allow URL/number input; one review-file write and one optional dprint call.
		const reviewPrompt = [
			...
			`PR number: ${prNumber}`,
			"Run only GitHub CLI commands needed for this exact flow:",
			`1) Generate diff with: gh pr diff ${prSelector}`,
			"2) Fetch PR title/body context with:",
			`   gh pr view --json title,body ${prSelector} --jq '. as $o | [\"title\", \"body\"] | map(\"## \\(.)\\n\\n\\($o[.])\\n\") | .[]'`,
			"Use the computed diff as source of truth. Use PR title/body as supporting context.",
			...(mode === "base-diff"
				? [
					"Assume the base branch is already checked out locally.",
					"Do not clone, fetch, checkout, or switch branches.",
				]
				: []),
			`Update ${reviewFileName} at project root with a first-pass review focused on glaring issues and risky changes.`,
			"Put the PR title at the very top as a Markdown H1: '# <PR title>'.",
			"Immediately below the title, include a quick PR health check: size (S/M/L/XL) and readiness (Ready/Needs Work/Blocked).",
			...
			"Add a 'Quick testing steps' section with 3-6 actionable smoke-test steps.",
			"Prefer testing steps explicitly mentioned in the PR body, and fill gaps from the diff when needed.",
	...
	pi.registerCommand("review", {
		...
		handler: async (args, ctx) => {
			await startReviewFlow(args, ctx, "default");
		},
	});

	pi.registerCommand("review-base-diff", {
		description:
			"Run one constrained review from 'gh pr diff' assuming the base branch is already checked out; no clone/checkout flow",
		handler: async (args, ctx) => {
			await startReviewFlow(args, ctx, "base-diff");
		},
	});
