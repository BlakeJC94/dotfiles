...
export function isGithubPrNumber(input: string): boolean {
...
export function isReviewDprintCommand(command: string, reviewFileName: string): boolean {
	const normalized = normalizeCommand(command);
	const escapedName = reviewFileName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
	return new RegExp(`^dprint (fmt|format) (\\./)?${escapedName}$`, "i").test(normalized);
}
