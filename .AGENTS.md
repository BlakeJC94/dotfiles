...
There will likely be a `PLAN` file that's named with the slugified current
branch name, use my git alias `git plan` to print the path to the file.

**Keep up to date with `PLAN`** --- it is the source of truth and the shared
append-only artifact between the developer and AI for communicating and
iterating on the project. Always consult it before giving advice or feedback so
your guidance aligns with what's been decided.
...
* `## Agent notes`: The ONLY heading where AI Agents and write to. The
    developer will inform agents if AI notes have been editing to
    correct/update understanding, and thus it should be re-read.

There may also be a `justfile` at the project root for common/documented
operations, invoked by a git alias `git just`
