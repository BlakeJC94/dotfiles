...
* **Keep up to date with `PLAN.md`** --- it is the source of truth and the
    shared artifact between the developer and AI for communicating and
    iterating on the project. Always consult it before giving advice or
    feedback so your guidance aligns with what's been decided. AI should only
    *update* PLAN.md when the developer specifically asks for it.
